import { a as getRequest, n as createMiddleware } from "./vendor_react-CFh17dx82.mjs";
import { t as createClient } from "../_libs/supabase__supabase-js.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/auth-middleware-B2BiZdKy.js
...
var requireSupabaseAuth = createMiddleware({ type: "function" }).server(async ({ next }) => {
	const { SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY, SUPABASE_ANON_KEY } = process.env;
	const supabaseKey = SUPABASE_PUBLISHABLE_KEY || SUPABASE_ANON_KEY;
	if (!SUPABASE_URL || !supabaseKey) {
		const missing = [];
		if (!SUPABASE_URL) missing.push("SUPABASE_URL");
		if (!supabaseKey) missing.push("SUPABASE_PUBLISHABLE_KEY / SUPABASE_ANON_KEY");
